...
class _LiveCallScreenState extends ConsumerState<LiveCallScreen> {
  ...
  void initState() {
    ...
    // Command the provider to start the VAD and WebSocket pipeline
  ...
  Future<void> _endCall(double finalRiskScore, {bool escalate = false}) async {
    // Command the provider to gracefully close the mic
    ...
    final newLog = RecordingLog(
      ...
      recordingName: 'Recording - ${DateFormat('MMM d, h:mm a').format(now)}',
      timestamp: now,
